  ...
  try {
    ...
    const [interestedLeads] = await pool.query(
      `SELECT COUNT(*) as count FROM leads
       WHERE assigned_to = ? AND status = 'Interested'`,
      [userId]
    );
    ...
    const totalAssignedCount = totalAssigned[0].count;
    const contactedCount = contactedLeads[0].count;

    // Correct conversion rate: conversions / contacted leads (not total assigned)
    const conversionRate = contactedCount > 0
      ? ((conversions[0].count / contactedCount) * 100).toFixed(2)
      : 0;
    ...
    res.json({
      ...
      data: {
        ...
        interestedLeads: interestedLeads[0].count,
    ...
    const comparisonData = await Promise.all(
      telecallers.map(async (telecaller) => {
        ...
        return {
          ...
          followupsMissed: missedFollowups[0].count,
          abuseFlag
        };
      })
    );
...
router.get('/daily-stats/history', async (req, res) => {
  try {
    ...
    let query = `
      ...
      WHERE ds.date >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
    `;
    ...
    // Calculate summary
    const flaggedDays = stats.filter(s => s.abuse_flag).length;
